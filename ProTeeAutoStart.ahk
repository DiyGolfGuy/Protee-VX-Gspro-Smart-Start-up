...
;@Ahk2Exe-SetName ProTee AutoStart
;@Ahk2Exe-SetDescription ProTee AutoStart - unattended golf sim startup
...
ShowBannerOn(mon) {
    global LogoWhite, QrImg, hBanner, gBannerMon, gBannerCloaked, gBannerVisible
    gBannerMon := mon                                   ; remember the GSPro monitor for FinishUp
    dpi := A_ScreenDPI / 96
    sw := mon.right - mon.left
    monH := mon.bottom - mon.top
    ; Cap the banner at 8% of the monitor height so low-res screens don't get a
    ; banner tall enough to sit on the menu tiles; f shrinks every element to fit.
    f := 1
    if (Round(132 * dpi) > Round(monH * 0.08))
        f := (monH * 0.08) / (132 * dpi)
    dpi := dpi * f
    bh := Round(132 * dpi)
    by := mon.bottom - bh
    ...
    padX  := Round(34 * dpi)
    gap   := Round(36 * dpi)
    fs1 := Max(8, Round(15 * f)), fs2 := Max(7, Round(12 * f))
    fs3 := Max(7, Round(11 * f)), fs4 := Max(8, Round(18 * f))
    ...
    ; Exclude the banner from screen captures (Win10 2004+): our own OCR then sees
    ; what's BEHIND the banner, so it can never block Play/PRACTICE on any screen.
    ; People still see the banner normally. On older Windows the call fails and
    ; OcrRegion falls back to briefly hiding the banner during each capture.
    gBannerCloaked := DllCall("SetWindowDisplayAffinity", "Ptr", hBanner, "UInt", 0x11) ? true : false
    gBannerVisible := true
    if (!gBannerCloaked)
        Log("Banner capture-exclusion unavailable on this Windows version - using hide-during-scan fallback.")
}
...
RunSequence() {
    ...
    Loop {
        ...
        ; --- 3) done: status good AND range (or menu) up -> click profile tab -> exit ---
        if ((rangeClicked || menuReady) && StatusGood(pt)) {
...
FinishUp(menuMode := false) {
    ...
    if (menuMode) {
        ; Stop-at-menu: click the empty strip near the bottom of the GSPro window,
        ; below the menu tiles, so the ProTee connector drops behind GSPro the same
        ; way it does when PRACTICE is clicked. No aim-reset here - not on the range.
        cx := gw.x + gw.w // 2
        cy := gw.y + Round(gw.h * 0.96)
        ClickAt(cx, cy)
        Sleep, 300
        MouseMove, % mon.right - 2, % mon.bottom - 2, 0
        Log("Final: stopped at the GSPro main menu, clicked below the tiles to drop the connector behind, parked cursor.")
        return
    }
...
OcrRegion(x, y, w, h) {
    global hBanner, gBannerCloaked, gBannerVisible
    o := {lines: [], text: ""}
    if (w <= 0 || h <= 0)
        return o
    ; Fallback for Windows versions without capture-exclusion: lift the banner out
    ; of the shot so OCR always reads what is behind it, then put it straight back.
    hid := false
    if (hBanner && gBannerVisible && !gBannerCloaked) {
        Gui, Banner:Hide
        hid := true
        Sleep, 60
    }
